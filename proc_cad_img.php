...
if ($SendCadImg) {
    ...
    $precocusto = filter_input(INPUT_POST, 'precocusto', FILTER_SANITIZE_STRING);
    ...
    //Inserir no BD
    $result_img = "INSERT INTO produto (nome, precocusto, precodesc, precovenda, qtd, info, imagem, imgprincipal, imgpqn1, imgpqn2, imgpqn, descricao, nometec) VALUES (:nome, :precocusto, :precodesc, :precovenda, :qtd, :info, :imagem, :imgprincipal, :imgpqn1, :imgpqn2, :imgpqn, :descricao, :nometec )";
    $insert_msg = $conn->prepare($result_img);
    $insert_msg->bindParam(':nome', $nome);
    $insert_msg->bindParam(':precocusto', $precocusto);
    $insert_msg->bindParam(':precodesc', $precodesc);
    $insert_msg->bindParam(':precovenda', $precovenda);
    $insert_msg->bindParam(':qtd', $qtd);
...
$insert_msg->bindParam(':nometec', $nometec);
    ...
    if ($insert_msg->execute()) {
        ...
        move_uploaded_file($_FILES['imgpqn']['tmp_name'], $diretorio.$nome_imgpqn)){
            $_SESSION['msg21'] = "<script> alert('Produto cadastrado com sucesso!'); </script>";
        ...
        header("Location: soprodutosadm.php");
    }
} else {
